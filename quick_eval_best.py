...
CKPT = "checkpoints/ckpt_15360.pth"
...
# reproducible seed
SEED = 0
trainer = PPOTrainer(env_id=ENV, seed=SEED, num_envs=1, device=device, total_steps=1, freeze_backbone=True)
...
for ep in range(EPISODES):
    ...
    try:
        ...
        while not done and steps < 5000:
            ...
            obs = next_obs
        logging.info(f"Episode {ep}: return={R:.2f}, steps={steps}")
        returns.append(R)
    except Exception as e:
...
logging.info(f"DET eval mean/std: {np.mean(returns):.2f} {np.std(returns):.2f}")
